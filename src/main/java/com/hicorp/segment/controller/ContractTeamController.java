...
import com.hicorp.segment.pojo.ContractTeam;
import com.hicorp.segment.security.utils.SecurityUtils;
import com.hicorp.segment.service.ContractTeamService;
...
@RestController
@RequestMapping("/contractTeam")
@Api(tags = "工厂数据管理")
public class ContractTeamController {

    private final ContractTeamService contractTeamService;
    private final SecurityUtils securityUtils;

    public ContractTeamController(SecurityUtils securityUtils, ContractTeamService contractTeamService) {
        this.securityUtils = securityUtils;
        this.contractTeamService = contractTeamService;
    }

    // 设备类型维护
    @Operation(summary = "获取设备类型维护信息", description = "带参数为分页查询设备类型维护，不带为查询所有")
    @GetMapping("/list")
    public ResultBean<List<ContractTeam>> getBasicDetailList() {
        return contractTeamService.queryInfo();
    }

    @Operation(summary = "查询设备类型维护信息", description = "带参数为分页查询设备类型维护信息，不带为查询所有")
    @GetMapping(value = "/list/{type}", params = {"page", "size", "sort"})
    public ResultBean<PageInfo<ContractTeam>> getBasicDetailList(@RequestParam("page") Integer page,
                                                                        @RequestParam("size") Integer size,
                                                                        @RequestParam("sort") String sort,
                                                                        @PathVariable("type") String type) {
        return contractTeamService.queryInfo(page, size, sort);
    }
    ...
    public ResultBean<PageInfo<ContractTeam>> getBasicDetailList(@RequestParam("page") Integer page,
        ...
        return contractTeamService.selectRecordsByPage(page, size, sort, field, content);
    }
    ...
    @PostMapping("/add")
    public ResultBean<Integer> addBasicDetail(@RequestBody ContractTeam basicDetail) {
        Date date = new Date();
        basicDetail.setCreateUser(securityUtils.getUserName());
        basicDetail.setCreateGmt(date);
        try {
            return contractTeamService.createRecord(basicDetail);
    ...
    @PostMapping("/addList")
    public ResultBean<Integer> addBasicDetailList(@RequestBody List<ContractTeam> deviceCategories) {
        ...
        return contractTeamService.createRecords(deviceCategories);
    }
    ...
    public ResultBean<Integer> deleteBasicDetail(@PathVariable(value = "id") Long id) {
        return contractTeamService.deleteRecord(id);
    }
    ...
    public ResultBean<Integer> deleteBasicDetailList(@RequestBody List<Object> ids) {
        return contractTeamService.deleteRecords("id", ids);
    }
    ...
    @Parameters({
            ...
            @Parameter(name = "basicDetail", description = "设备类型维护信息实体", required = true)})
    @PutMapping("/{id}")
    public ResultBean<Integer> updateBasicDetail(@PathVariable(value = "id") Long id, @RequestBody ContractTeam basicDetail) {
        Date date = new Date();
        basicDetail.setId(id);
        basicDetail.setModifiedUser(securityUtils.getUserName());
        basicDetail.setModifiedGmt(date);
        try {
            return contractTeamService.updateNotNull(basicDetail);
        } catch (Exception e) {
