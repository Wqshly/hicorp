...
import com.hicorp.segment.pojo.Factory;
import com.hicorp.segment.security.utils.SecurityUtils;
import com.hicorp.segment.service.FactoryService;
...
public class FactoryController {
    ...
    public FactoryController(SecurityUtils securityUtils, FactoryService factoryService) {
    ...
    @GetMapping("/list")
    public ResultBean<List<Factory>> getBasicDetailList() {
        return factoryService.queryInfo();
    }

    @Operation(summary = "查询设备类型维护信息", description = "带参数为分页查询设备类型维护信息，不带为查询所有")
    @GetMapping(value = "/list/{type}", params = {"page", "size", "sort"})
    public ResultBean<PageInfo<Factory>> getBasicDetailList(@RequestParam("page") Integer page,
                                                                        @RequestParam("size") Integer size,
                                                                        @RequestParam("sort") String sort,
                                                                        @PathVariable("type") String type) {
        return factoryService.queryInfo(page, size, sort);
    }
    ...
    public ResultBean<PageInfo<Factory>> getBasicDetailList(@RequestParam("page") Integer page,
        ...
        return factoryService.selectRecordsByPage(page, size, sort, field, content);
    }
    ...
    @PostMapping("/add")
    public ResultBean<Integer> addBasicDetail(@RequestBody Factory basicDetail) {
        Date date = new Date();
        basicDetail.setCreateUser(securityUtils.getUserName());
        basicDetail.setCreateGmt(date);
        try {
            return factoryService.createRecord(basicDetail);
    ...
    @PostMapping("/addList")
    public ResultBean<Integer> addBasicDetailList(@RequestBody List<Factory> deviceCategories) {
        ...
        return factoryService.createRecords(deviceCategories);
    }
    ...
    public ResultBean<Integer> deleteBasicDetail(@PathVariable(value = "id") Long id) {
        return factoryService.deleteRecord(id);
    }
    ...
    public ResultBean<Integer> deleteBasicDetailList(@RequestBody List<Object> ids) {
        return factoryService.deleteRecords("id", ids);
    }
    ...
    @Parameters({
            ...
            @Parameter(name = "basicDetail", description = "设备类型维护信息实体", required = true)})
    @PutMapping("/{id}")
    public ResultBean<Integer> updateBasicDetail(@PathVariable(value = "id") Long id, @RequestBody Factory basicDetail) {
        Date date = new Date();
        basicDetail.setId(id);
        basicDetail.setModifiedUser(securityUtils.getUserName());
        basicDetail.setModifiedGmt(date);
        try {
            return factoryService.updateNotNull(basicDetail);
        } catch (Exception e) {
