package com.hicorp.segment.service.impl;

import com.hicorp.segment.pojo.BasicDetailData;
import com.hicorp.segment.service.BasicDetailDataService;
import org.springframework.stereotype.Service;

/**
 * @Author:
 * @Date: Created in 15:13 2021/5/25
 * @Description:
 * @ChineseDescription:
 * @Modified_By:
 */
@Service("basicDetailDataServiceImpl")
public class BasicDetailDataServiceImpl extends BasicInterfaceImpl<BasicDetailData> implements BasicDetailDataService {
}
